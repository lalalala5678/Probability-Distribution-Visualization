        ...
        switch (dist) {
            case 'bernoulli':
                ...
                formulaDiv.innerHTML = `
                    ...
                    <p>概率质量函数：\\[ P(X = 1) = p, \\; P(X = 0) = 1 - p \\]</p>
                    <p>期望：\\[ E(X) = p \\]</p>
                    <p>方差：\\[ \\mathrm{Var}(X) = p(1 - p) \\]</p>
                    <p>推导：伯努利分布是二项分布在一次试验中的特例，即\\( n = 1 \\)。</p>
                `;
                break;
            case 'binomial':
                params = [
                ...
                formulaDiv.innerHTML = `
                    ...
                    <p>概率质量函数：\\[ P(X = k) = \\binom{n}{k} p^k (1 - p)^{n - k} \\]</p>
                    <p>期望：\\[ E(X) = np \\]</p>
                    <p>方差：\\[ \\mathrm{Var}(X) = np(1 - p) \\]</p>
                    <p>推导：二项分布可以通过多次独立伯努利试验的和来推导。</p>
                `;
                break;
            case 'poisson':
                params = [
                ...
                formulaDiv.innerHTML = `
                    ...
                    <p>概率质量函数：\\[ P(X = k) = \\frac{\\lambda^k e^{-\\lambda}}{k!} \\]</p>
                    <p>期望：\\[ E(X) = \\lambda \\]</p>
                    <p>方差：\\[ \\mathrm{Var}(X) = \\lambda \\]</p>
                    <p>推导：泊松分布是二项分布在事件发生概率趋于零且试验次数趋于无穷大时的极限分布。</p>
                `;
                break;
            case 'uniform':
                params = [
                ...
                formulaDiv.innerHTML = `
                    ...
                    <p>概率密度函数：\\[ f(x) = \\frac{1}{b - a}, \\; a \\leq x \\leq b \\]</p>
                    <p>期望：\\[ E(X) = \\frac{a + b}{2} \\]</p>
                    <p>方差：\\[ \\mathrm{Var}(X) = \\frac{(b - a)^2}{12} \\]</p>
                    <p>推导：均匀分布的概率密度函数是由其定义直接得出的。</p>
                `;
                break;
            case 'exponential':
                params = [
                ...
                formulaDiv.innerHTML = `
                    ...
                    <p>概率密度函数：\\[ f(x) = \\lambda e^{-\\lambda x}, \\; x \\geq 0 \\]</p>
                    <p>期望：\\[ E(X) = \\frac{1}{\\lambda} \\]</p>
                    <p>方差：\\[ \\mathrm{Var}(X) = \\frac{1}{\\lambda^2} \\]</p>
                    <p>推导：指数分布是泊松过程中的时间间隔分布。</p>
                `;
                break;
            case 'gaussian':
                params = [
                ...
                formulaDiv.innerHTML = `
                    ...
                    <p>概率密度函数：\\[ f(x) = \\frac{1}{\\sigma \\sqrt{2\\pi}} e^{-\\frac{(x - \\mu)^2}{2\\sigma^2}} \\]</p>
                    <p>期望：\\[ E(X) = \\mu \\]</p>
                    <p>方差：\\[ \\mathrm{Var}(X) = \\sigma^2 \\]</p>
                    <p>推导：高斯分布可以通过中心极限定理得出，表示大量独立随机变量之和的分布。</p>
                `;
                break;
        }
